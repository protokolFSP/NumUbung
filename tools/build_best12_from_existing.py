...
# =========================================
# file: .github/workflows/build_best12.yml
# =========================================
# (Optional) Run this after you already have clips in outputs/clips
"""
name: Build best 12 Übung from existing clips

on:
  workflow_dispatch:
    inputs:
      count:
        description: "Kaç klip seçilsin"
        required: true
        default: "12"
      silence_seconds:
        description: "Klipler arası sessizlik"
        required: true
        default: "0.6"
      ...
      - name: Build best12
        run: |
          python tools/build_best12_from_existing.py \
            ...
            --out-dir outputs/ubung \
            --count "${{ inputs.count }}" \
            --silence-seconds "${{ inputs.silence_seconds }}"

      - name: Commit & push outputs
        run: |
          git config user.name "github-actions[bot]"
          git config user.email "github-actions[bot]@users.noreply.github.com"
          git add outputs/ubung
          git commit -m "Add best12 Übung" || exit 0
          git pull --rebase
          git push
"""
